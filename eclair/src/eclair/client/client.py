...
class EclairClient:
    ...
    async def search_datasets(self, query: str):
        ...
        return await self.call_mcp_tool("search-datasets", {"query": query})

    async def download_dataset(self, collection: str, dataset: str):
        """Download a dataset."""
        return await self.call_mcp_tool("download-dataset", {"collection": collection, "dataset": dataset})

    async def datasets_preview_url(self, collection: str, dataset: str):
        """Get a download URL for a dataset preview."""
        return await self.call_mcp_tool("datasets-preview-url", {"collection": collection, "dataset": dataset})

    async def serve_croissant(self, collection: str, dataset: str):
        """Get the Croissant dataset metadata."""
        return await self.call_mcp_tool("serve-croissant", {"collection": collection, "dataset": dataset})

    async def validate_croissant(self, metadata_json: Dict[str, Any]):
        """Validate a Croissant metadata file."""
        return await self.call_mcp_tool("validate-croissant", {"metadata_json": metadata_json})

    async def get_help(self):
        """Get help for the MLC Bakery API."""
        return await self.call_mcp_tool("help")
